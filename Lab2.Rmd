# Week 2  Exploratory Data Analysis {#wk2}
...
```{r}
# data <- read.csv("birthweight_smoking.csv",header=TRUE, sep=",") # for the *cloud* version
...
```
Describe the minimum and maximum of the dummy variable here: __________
...
summary(data)
# An intuitive but two step approach: 
...
**Subset the data** by specifying criteria for pulling a partial data from the `data` based on the value of a variable or the values of two variables. See other operators at the bottom of the page. 
```{r}
summary(data)

# This new dataset will now consist of smoking mothers *only*. 
data_smoker <- subset(data, data$smoker == 1)

# This new dataset will now consist of mothers whose age is or above 18 years old *only*. 
data_adult <- subset(data, data$age >= 18) 

# This new dataset will now consist of smoking mothers whose age is or above 18 years old *only*. 
data_smoker_adult <- subset(data, data$smoker == 1 & data$age >= 18)  

```
Try it yourself! Remember to specify a new dataframe name: `new_data`. If you put data at the left side of the arrow, you will replace your existing `data`.
If you mess up the existing data, try import the csv file again from the beginning. 
